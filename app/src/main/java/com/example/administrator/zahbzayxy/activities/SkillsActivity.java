...
public class SkillsActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_skills_layout);
        initView();
        initToken();
        initOnClick();
    }
    ...
    private void initOnClick() {
        queDingName_bt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    occupaSkillLevel = nameSet_et.getText().toString();
                    param.put("occupaSkillLevel", occupaSkillLevel);
                ...
                updateUserNameData.enqueue(new Callback<UserInfoResetBean>() {
                    ...
                    public void onResponse(Call<UserInfoResetBean> call, Response<UserInfoResetBean> response) {
                        ...
                        if (body != null) {
                            ...
                            if (data == true) {
                                Toast.makeText(SkillsActivity.this, "职业工种等级修改成功", Toast.LENGTH_SHORT).show();
                                //把姓名回传到用户中心
                                Intent intent = new Intent(SkillsActivity.this, EditMessageActivity.class);
                                intent.putExtra("occupaSkillLevel", occupaSkillLevel);
                                setResult(RESULT_OK, intent);
                                SkillsActivity.this.finish();
                                EventBus.getDefault().post(occupaSkillLevel);
                            } else {
                                Toast.makeText(SkillsActivity.this, "修改失败，请稍后重试", Toast.LENGTH_SHORT).show();
                            }
                        }else{
                            Toast.makeText(SkillsActivity.this, "请求服务器失败", Toast.LENGTH_SHORT).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<UserInfoResetBean> call, Throwable t) {
                        Toast.makeText(SkillsActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                    }
                });
