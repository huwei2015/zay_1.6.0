...
/**
 * Created by ${ZWJ} on 2017/2/7 0007.
 */
public class LessonSecondGVAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        Picasso.with(context).load(list.get(position).getCourseImagePath()).placeholder(R.mipmap.loading_png).into(viewHold.lessonName_iv);
        LessonSecondGridViewBean.DataBean.CourseListBean courseListBean = list.get(position);
        String courseName = courseListBean.getCourseName();
        int courseHours = courseListBean.getCourseHours();
        String teacher = courseListBean.getTeacher();
