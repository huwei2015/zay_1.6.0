...
public class FaceRecognitionForOfflineActivity extends FaceLivenessActivity {
...
//                initDownLoadData();
        ...
        if (ContextCompat.checkSelfPermission(FaceRecognitionForOfflineActivity.this,Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(FaceRecognitionForOfflineActivity.this, new String[]{Manifest.permission.CAMERA}, 1000);
        }
...
//            if("bestImage0".equals(entry.getKey().toString())){
//                continue;
//            }else{
...
//            }
//        }
//        bmp = base64ToBitmap(imageMap.get("bestImage0"));
    ...
    class UploadFaceRecognitionRunnable implements Runnable {
        ...
        public void run() {
            try{
                ...
                selectionId = getIntent().getIntExtra("selectId",0);
                currentPosition = getIntent().getIntExtra("currentPosition", 0);
                Log.i("hw","============="+selectionId);
                ...
                if(fact_img==null){
                    ...
                    Request request = new Request.Builder()
                            ...
                            .post(requestBody)
                            .build();
                    Log.i("request人脸识别","request=================="+request);
            ...
            switch (rootIn){
                ...
                case 1:
                    if (isSuccess){
                        ...
                        FaceRecognitionForOfflineActivity.this.finish();
                    }
                    break;
                case 2:
                    if (isSuccess){
//                        setResult(200);
    ...
    private void goToMediaPlayActivity(int rootIn){
        Intent intent = new Intent(FaceRecognitionForOfflineActivity.this, MediaPlayActivity.class);
        Bundle bundle = new Bundle();
        intent.putExtra("videoId", videoId);
        intent.putExtra("isLocalPlay", true);
        intent.putExtra("userCourseId",userCourseId);
        intent.putExtra("selectionId",selectionId);
...
//        bundle.putInt("sectionId",selectionId);
...
//        isLocalPlay = getIntent().getBooleanExtra("isLocalPlay",false);
        ...
        aClass.getPMyLessonPlayData(courseId,token,userCourseId).enqueue(new Callback<PMyLessonPlayBean>() {
            @Override
            public void onResponse(Call<PMyLessonPlayBean> call, Response<PMyLessonPlayBean> response) {
                ...
                if (body!=null){
                    if (body.getErrMsg()==null){
                                    ...
                                    for (int j=0;j<size1;j++){
                                        ...
                                        for (int h=0;h<size2;h++){
                                            ...
                                            if (currPlay==true){
                                                ...
                                                playTime = selectionListBean.getPlayTime();
                                            }
                                        }
                                    }
                                }
                        ...
                        Log.i("dfdfdfd",errMsg+"");
                    }
                }
            }
            @Override
            public void onFailure(Call<PMyLessonPlayBean> call, Throwable t) {
                Toast.makeText(FaceRecognitionForOfflineActivity.this,"网路问题",Toast.LENGTH_SHORT).show();
            }
        });
    }
