    ...
    private void initOnClick() {
        queDingName_bt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    quarters = nameSet_et.getText().toString();
                ...
                updateUserNameData.enqueue(new Callback<UserInfoResetBean>() {
                    ...
                    public void onResponse(Call<UserInfoResetBean> call, Response<UserInfoResetBean> response) {
                        ...
                        if (body != null) {
                            ...
                            if (data == true) {
                                Toast.makeText(jobsActivity.this, "岗位修改成功", Toast.LENGTH_SHORT).show();
                                //把姓名回传到用户中心
                                Intent intent = new Intent(jobsActivity.this, EditMessageActivity.class);
                                intent.putExtra("quarters", quarters);
                                setResult(RESULT_OK, intent);
                                jobsActivity.this.finish();
                                EventBus.getDefault().post(quarters);
                            } else {
                                Toast.makeText(jobsActivity.this, "修改失败，请稍后重试", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                    ...
                    public void onFailure(Call<UserInfoResetBean> call, Throwable t) {
                        Toast.makeText(jobsActivity.this, "请求服务器失败", Toast.LENGTH_SHORT).show();
                    }
                });
