...
//        //加载popupwindow的布局文件
//        View view1 = View.inflate(UpdateCertificateActivity.this, R.layout.activity_pop_one, null);
//        cancle = view1.findViewById(R.id.mine_pop_cancle);
//        RelativeLayout camera = view1.findViewById(R.id.mine_pop_camera);
//        RelativeLayout selector = view1.findViewById(R.id.mine_pop_selector);
//        //创建popupwindow为全屏
//
...
//                        uri = FileProvider.getUriForFile(UpdateCertificateActivity.this, UpdateCertificateActivity.this.getApplicationContext().getPackageName() + ".fileprovider", new File(path));
//                    } else {
...
//    }
    ...
    private void initView() {
        ...
        back_editMessage = (ImageView) findViewById(R.id.back_editMessage);
        back_editMessage.setOnClickListener(this);
        img_one_cun = (ImageView) findViewById(R.id.img_one_cun);//xuel zhengshi
        btn_photo = (Button) findViewById(R.id.btn_photo);
        btn_photo.setOnClickListener(this);
        UserInfoInterface userInfoInterface = RetrofitUtils.getInstance().createClass(UserInfoInterface.class);
        Call<UserInfoBean> userInfoData = userInfoInterface.getUserInfoData(token);
        userInfoData.enqueue(new Callback<UserInfoBean>() {
            ...
            public void onResponse(Call<UserInfoBean> call, Response<UserInfoBean> response) {
                if(response !=null && response.body() !=null){
                    ...
                    String eduCerPath = data.getEduCerPath();
                    if(!TextUtils.isEmpty(eduCerPath)){
                        Picasso.with(UpdateCertificateActivity.this).load(eduCerPath).placeholder(R.mipmap.timg).into(img_one_cun);
    ...
    private void updatePhoto(byte[] url) {
        ...
        aClass.getCertificateData(body,token).enqueue(new Callback<UpBean>() {
            ...
            public void onResponse(Call<UpBean> call, Response<UpBean> response) {
                if(response !=null && response.body() !=null){
                    UpBean body = response.body();
                    UpBean.OneCunBean data = body.getData();
                    String oneInchPhoto = data.getPhotoUrl();
...
//                        String photoUrl = response.body().getData().getPhonePath();
//                        Log.e("photoUrlphotoUrl", photoUrl);
//                        if (!TextUtils.isEmpty(photoUrl)) {
//                            EventBus.getDefault().post(photoUrl);
//                            Picasso.with(UpdateCertificateActivity.this).load(photoUrl).placeholder(R.mipmap.timg).into(img_one_cun);
//                            btn_photo.setText("重新上传照片");
//                        } else {
//                            img_one_cun.setVisibility(View.GONE);
...
//                        ToastUtils.showInfo(response.body().getErrMsg(),500);
//                    }
//                }
            }
...
//                    }
                    ...
                    updatePhoto(this.bitmapByte);
                    img_one_cun.setVisibility(View.VISIBLE);
                    Toast.makeText(UpdateCertificateActivity.this,"图片上传成功",Toast.LENGTH_SHORT).show();
                    break;
            }
        }
    }
